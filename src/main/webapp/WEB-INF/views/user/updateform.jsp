...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
	UserVo viewImfor = (UserVo)request.getAttribute("viewImfor");
		...
		<div id="content">
			<div id="user">
				<form id="join-form" name="joinForm" method="post" action="<%=request.getContextPath()%>/user">
					...
					<input id="name" name="name" type="text" value="<%=viewImfor.getName()%>">

					<label class="block-label" for="email">이메일</label>
					<h4><%=viewImfor.getEmail()%></h4>

					<label class="block-label">패스워드</label> <input name="password" type="password" value="">

					<fieldset>
						<%
							if ("male".equals(viewImfor.getGender())) {
						%>
						<legend>성별</legend>
